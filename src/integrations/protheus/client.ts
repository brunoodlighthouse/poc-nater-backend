...
export async function protheusRequest(opts: ProtheusRequestOptions): Promise<ProtheusResponse> {
  if (circuitBreaker.isOpen()) {
    logger.warn(
      { correlationId: opts.correlationId, path: opts.path, cbState: circuitBreaker.getState() },
      'Circuit breaker aberto - requisicao ao Protheus bloqueada',
    );
