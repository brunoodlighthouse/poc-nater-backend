...
function buildServerUrl(request: FastifyRequest): string {
...
export function buildErrorResponses(statuses: number[]) {
  return Object.fromEntries(statuses.map((status) => [status, errorResponseRef]));
}

export function okResponseSchema(data: Record<string, unknown>) {
  return {
    type: 'object',
    required: ['ok', 'data'],
    properties: {
...
export const swaggerOptions: FastifyDynamicSwaggerOptions = {
  openapi: {
    ...
    tags: [
      ...
      { name: 'Documentos', description: 'Consulta e listagem de documentos' },
      { name: 'Entregas', description: 'Execucao e finalizacao de entregas' },
      { name: 'Webhooks', description: 'Webhooks de integracao com Protheus' },
...
export function registerSwaggerSchemas(app: FastifyInstance<any, any, any, any, any>) {
  app.addSchema({
    $id: 'ErrorResponse',
    type: 'object',
    required: ['ok', 'error'],
