...
import { createCourierGateway } from './integrations/protheus/entregadores.js';
import { createDeliveryGateway } from './integrations/protheus/entregas.js';
import { createStoreGateway } from './integrations/protheus/filiais.js';
...
export function buildApp() {
    ...
    const storeGateway = createStoreGateway();
    const documentGateway = createDocumentGateway();
    const courierGateway = createCourierGateway();
    ...
    app.get('/health/ready', async (_request, reply) => {
        ...
        return { ok: true, db: 'up', protheus: config.PROTHEUS_MOCK_ENABLED ? 'mock' : 'not_checked' };
    });
